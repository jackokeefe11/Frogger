...
bool
Frog::hits_side(Game_config const& config) const
{
    return (top_left.x + config.frog_dims.width < 0 ||
            top_left.x > config.scene_dims.width);
}
...
Frog::next(double dt) const
{
    ...
    if (result.velocity.width != 0) {
        temp.right_by(d * result.velocity.width);
        result.top_left = temp;
    }
    return result;
}
